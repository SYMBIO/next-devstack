...
function renderPagination(page: number, total: number, callback: (page: number) => void): ReactElement[] {
    ...
    for (let p = start; p <= end; p++) {
        if (p === page) {
            result.push(
                <li key={`PaginationItem-${p}`} className={condCls(styles.linkItem, styles.linkItemActive)}>
                    {p}
                </li>,
            );
        } else {
...
export const Pagination = ({ className, page = 1, total = 0, callback }: PaginationProps): ReactElement | null => {
    if (total <= 1) {
        return null;
    }
