...
async function job() {
    for (const report of annualReportsCs.slice(9)) {
        const enReport = annualReportsEn.find((enReport) => enReport.publicated === report.publicated);
        ...
        await client.items.create({
            ...
            date:
                report.financial_reports_category_id === '3'
                    ? dayjs(report.publicated).format()
                    : dayjs(report.publicated).subtract(1, 'year').format(),
            file: {
                cs: {
